...
// had to copy other stuff due to sealed abstract classes
...
final class StreamZipArchive(override val name : String,override val path : String, override val input : InputStream) extends ZipArchive(null) {
  def iterator = {
    val root     = new DirEntry("/")
    val dirs     = mutable.HashMap[String, DirEntry]("/" -> root)
    val in       = new ZipInputStream(input)
  ...
  override def hashCode() = input.hashCode
